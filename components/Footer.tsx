...
export default function Footer() {
  return (
    <footer className="bg-[#F8F4F9] border-t border-brand-accent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* Connect */}
          <div>
            <h4 className="font-semibold text-[#2D2D2D] mb-4">Connect</h4>
            <div className="space-y-2">
              <a
                href={`https://instagram.com/${process.env.NEXT_PUBLIC_INSTAGRAM_HANDLE}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center space-x-2 text-[#666666] hover:text-[#2D2D2D] transition-colors text-sm"
              >
                ...
                <span>@{process.env.NEXT_PUBLIC_INSTAGRAM_HANDLE}</span>
              </a>
            </div>
          </div>
        </div>

        <div className="mt-8 pt-8 border-t border-brand-accent text-center text-sm text-[#666666]">
          <p>© {new Date().getFullYear()} Minn Arc. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
